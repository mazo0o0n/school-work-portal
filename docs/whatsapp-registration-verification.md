...
هذه الوثيقة تصف أساس التحقق من رقم جوال مسؤول تسجيل المدرسة عبر WhatsApp Cloud API. التحقق اختياري، ويبقى معطلًا ما لم تكن قيمة `PHONE_VERIFICATION_REQUIRED` مساوية حرفيًا لـ`true`. إرسال OTP له مفتاح مستقل، ويبقى معطلًا ما لم تكن قيمة `OTP_SEND_ENABLED` مساوية حرفيًا لـ`true`.
...
3. عند تفعيل التحقق والإرسال، يرسل العميل الرقم إلى `POST /api/register/send-whatsapp-code`.
...
قبل أي اتصال بمزود WhatsApp، يحجز الخادم محاولة إرسال في `phone_verification_abuse_events` باستخدام `INSERT ... SELECT` ذري. الحدود الحالية: 3/10 دقائق و6/ساعة و10/24 ساعة للهاتف، 20/ساعة و50/24 ساعة لـIP المرمّز، 3/10 دقائق للمفتاح المركب، و100 محاولة مزود/24 ساعة عالميًا مع تحذير عند 80. كل محاولة مزود محجوزة تبقى محسوبة حتى عند فشل المزود.
...
- `OTP_SEND_ENABLED`: يسمح بإرسال OTP فقط عند القيمة `true`. القيمة الغائبة أو أي قيمة أخرى تعني تعطيل الإرسال، دون تعطيل اشتراط verification token.
...
- عدادات الإساءة تخزن hashes مملحة للهاتف وIP والمفتاح المركب. لا تخزن IP خامًا.
...
- **Production:** Worker الإنتاج وbindings الحالية دون تغيير. يبقى WhatsApp معطلًا ما لم تكن قيمتا `PHONE_VERIFICATION_REQUIRED` و`OTP_SEND_ENABLED` مساويتين حرفيًا لـ`true`.
...
- `PHONE_VERIFICATION_REQUIRED=false` في أول نشر، وغياب `OTP_SEND_ENABLED` يعني أن الإرسال معطل أيضًا.
...
بعد نجاح النشر المعطل، تُفعّل `PHONE_VERIFICATION_REQUIRED=true` و`OTP_SEND_ENABLED=true` في Preview وحده، ويضاف رقم الاختبار تفاعليًا إلى `WHATSAPP_TEST_ALLOWED_PHONES`. لا يُطلب إدخال أي سر في المحادثة أو في سطر أوامر ظاهر.
...
لإيقاف تكلفة الإرسال مع إبقاء التسجيل محميًا، اضبط `OTP_SEND_ENABLED=false` مع إبقاء `PHONE_VERIFICATION_REQUIRED=true`. لا يتطلب الكود قيمة افتراضية في `wrangler.toml`؛ غياب `OTP_SEND_ENABLED` يعطل الإرسال بأمان. التفعيل المستقبلي يتطلب ضبطه صراحة في البيئة المقصودة بعد تطبيق migration وموافقة المالك.
...
مكتمل برمجيًا: توليد OTP آمن، تخزين hash، حدود الصلاحية والمحاولات، token أحادي الاستخدام، إبطال الرمز بعد فشل المزود، timeout، Graph API version، التحقق من إعدادات القالب، قائمة السماح، مفتاح اشتراط التحقق، مفتاح إرسال مستقل، حدود D1 طويلة المدى، حد تكلفة عالمي، حدود تحقق عابرة للتحديات، حجز ذري، وحظر خمس دقائق بعد فشل شبكة غامض.
